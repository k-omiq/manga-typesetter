...
to over HTTP. It hosts one thing: text detection + OCR (mokuro /
comic-text-detector + manga-ocr), which feeds the typesetting queue.
...
- `GET  /health` — status, version, compute device, model dir
- `POST /analyze` — detection + OCR → lines in JP reading order
- `GET  /models/cache`, `POST /models/cache/clear` — weight cache size / purge
