...
describe('sanitiseBrushSettings', () => {
  ...
  it('keeps the opacity and thickness dynamics on the same terms, and no key when absent', () => {
    const s = sanitiseBrushSettings({
      dynOpacity: { src: 'pressure', amount: 80 },
      dynThick: { src: 'random', amount: 76, curve: 'junk' },
    });
    expect(s.dynOpacity).toEqual({ src: 'pressure', amount: 80 });
    expect(s.dynThick).toEqual({ src: 'random', amount: 76 });
    expect(s.dyn).toBeUndefined();
    const none = sanitiseBrushSettings({ size: 30 });
    expect('dynOpacity' in none).toBe(false);
    expect('dynThick' in none).toBe(false);
  });
...
describe('sanitiseBrushSettings, phase 7.2', () => {
  it('reads the grade, the flips, the blend, the grain and the two switches', () => {
    const s = sanitiseBrushSettings({
      antialias: 1, flipX: 'on', flipY: 'random', blend: 'density',
      texture: { on: true, density: 1, scale: 300, stress: true },
      stabiliseBySpeed: true, postBezier: true,
    });
    expect([s.antialias, s.flipX, s.flipY, s.blend]).toEqual([1, 'on', 'random', 'density']);
    expect(s.texture).toEqual({ on: true, density: 1, scale: 300, stress: true });
    expect([s.stabiliseBySpeed, s.postBezier]).toEqual([true, true]);
  });

  it('reads an index row from before any of it as the engine defaults', () => {
    const s = sanitiseBrushSettings({ size: 40, antialias: true });
    expect([s.antialias, s.flipX, s.flipY, s.blend]).toEqual([3, 'off', 'off', 'over']);
    expect(s.texture).toEqual({ on: false, density: 0.5, scale: 100, stress: false });
    expect([s.stabiliseBySpeed, s.postBezier]).toEqual([false, false]);
    expect(sanitiseBrushSettings({ antialias: false }).antialias).toBe(0);
    expect(sanitiseBrushSettings({ flipX: 'sideways', blend: 'screen', texture: 'yes' }).texture.on).toBe(false);
  });
});
