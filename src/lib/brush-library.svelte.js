...
import { defaultBrushSettings, dynCurve, DYN_SOURCES, FLIP_MODES, BLEND_MODES, aaLevel, normalizeTexture } from './brush.js';
...
export function sanitiseBrushSettings(src) {
  ...
  // The opacity and thickness drivers follow the same contract as `dyn`:
  // present only when the file (or the manager) had an opinion.
  const dynOpacity = importedDyn(s.dynOpacity);
  const dynThick = importedDyn(s.dynThick);
  return {
    ...(dyn ? { dyn } : null),
    ...(dynOpacity ? { dynOpacity } : null),
    ...(dynThick ? { dynThick } : null),
    ...
    flipX: FLIP_MODES.includes(s.flipX) ? s.flipX : 'off',
    flipY: FLIP_MODES.includes(s.flipY) ? s.flipY : 'off',
    flatness: num(s.flatness, d.flatness, 0.01, 1),
    antialias: aaLevel(s.antialias),
    blend: BLEND_MODES.includes(s.blend) ? s.blend : 'over',
    texture: normalizeTexture(s.texture),
    ...
    stabiliseBySpeed: s.stabiliseBySpeed === true,
