...
describe('textLayerFor', () => {
  ...
  it('attaches a stroke effect with the correct size, position, and color when outlineWidth > 0', () => {
    const layer = textLayerFor(
      page,
      box({ ...baseStyle, outlineWidth: 3, outline: '#ff3300' }),
      rendered,
    );
  ...
  it('leaves stroke off when outlineWidth is zero or absent', () => {
    const layerZero = textLayerFor(page, box({ ...baseStyle, outlineWidth: 0, outline: '#ff3300' }), rendered);
  ...
  it('leaves dropShadow off when shadow.on is false or shadow is absent', () => {
    const layerOff = textLayerFor(
      page,
      box({ ...baseStyle, shadow: { on: false, x: 5, y: 5, blur: 4, color: '#000000', opacity: 0.8 } }),
      rendered,
    );
  ...
  it('attaches a dropShadow effect and round-trips angle and distance back to canvas x and y offsets', () => {
    ...
    for (const s of testCases) {
      const layer = textLayerFor(
        page,
        box({ ...baseStyle, shadow: { on: true, ...s } }),
        rendered,
      );
  ...
  it('leaves raster-only layers (curve, roughen, flip) completely free of effects', () => {
    const styledWithEffects = {
      ...
      outlineWidth: 4,
      outline: '#ffffff',
      shadow: { on: true, x: 3, y: 4, blur: 5, color: '#000000', opacity: 0.8 },
    };
  ...
  it('round-trips stroke and drop shadow layer effects through a real PSD', () => {
    ...
    const styledBox = {
      ...
      style: {
        ...
        outlineWidth: 3,
        outline: '#ff0000',
        shadow: { on: true, x: 4, y: 5, blur: 6, color: '#000000', opacity: 0.75 },
      },
    };
    const renderedBox = {
      imageData: noise(50, 30, 99),
