...
describe('command records', () => {
  ...
  it('undoes and redoes a resize of an ink box, restoring stroke geometry', () => {
    const b = page().boxes[0];
    const originalInk = {
      on: true,
      strokes: [{ brush: 'round', size: 10, pts: [[5, 10, 1], [55, 10, 1]] }],
    };
    const scaledInk = {
      on: true,
      strokes: [{ brush: 'round', size: 20, pts: [[10, 20, 1], [110, 20, 1]] }],
    };
    b.w = 120;
    b.h = 40;
    b.style.ink = structuredClone(scaledInk);
    record({
      t: 'resize',
      pageId: 1,
      boxId: 'b1',
      before: { x: 10, y: 10, w: 60, h: 20, ink: structuredClone(originalInk) },
      after: { x: 10, y: 10, w: 120, h: 40, ink: structuredClone(scaledInk) },
    });
    undo();
    expect([b.w, b.h]).toEqual([60, 20]);
    expect(b.style.ink.strokes[0].size).toBe(10);
    expect(b.style.ink.strokes[0].pts).toEqual([[5, 10, 1], [55, 10, 1]]);
    redo();
