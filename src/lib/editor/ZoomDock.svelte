<script>
  ...
  import { app, setZoom, zoomReset } from '../store.svelte.js';
...
<div class="zoomdock">
  ...
  <span class="sep"></span>
  <button onclick={() => setZoom(app.zoom / 1.2)} aria-label="Zoom out">−</button>
  <button class="zval" onclick={zoomReset} data-tip="Reset to 100%">{zoomPct}%</button>
  <button onclick={() => setZoom(app.zoom * 1.2)} aria-label="Zoom in">+</button>
  <span class="sep"></span>
