...
describe('the point factors under liquify', () => {
  it('are interpolated by the resample and kept by the push', () => {
    const k = { brush: 'round', size: 10, pts: [[0, 0, 1, 1, 1], [100, 0, 0, 0, 0]] };
    const dense = resampleStroke(k, 50);
    expect(dense.pts).toHaveLength(3);
    expect(dense.pts[1]).toEqual([50, 0, 0.5, 0.5, 0.5]);
    const [out] = applyLiquify([k], { mode: 'push', cx: 100, cy: 0, radius: 30, strength: 100, dx: 5, dy: 5 });
    expect(out).not.toBe(k);
    expect(out.pts.at(-1).slice(2)).toEqual([0, 0, 0]);
    expect(out.pts.at(-1)[0]).toBeGreaterThan(100);
  });
});
