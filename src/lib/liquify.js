...
// stay leaves. The two point helpers come from the engine so a moved point
// keeps every factor the dynamics gave it.
import { mixPoint, movePoint } from './brush.js';
...
export function applyLiquify(strokes, opts) {
  ...
  return list.map((stroke) => {
    ...
    const pts = dense.pts.map((p) => {
      ...
      // points are, so liquify never touches the factors.
      const vx = x - cx;
      const vy = y - cy;
      const d = Math.hypot(vx, vy);
      const w = falloff(d, r);
      if (w <= 0) return movePoint(p, x, y);
      const k = w * s;
      let nx = x;
      let ny = y;
      if (push) {
