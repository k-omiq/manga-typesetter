...
describe('ink style block', () => {
  ...
  it('keeps anti-aliasing at Strong for a stroke saved before the switch existed', () => {
    expect(normalizeInkStroke({ pts: [[1, 2, 1]] }).antialias).toBe(3);
    expect(normalizeInkStroke({ antialias: true, pts: [[1, 2, 1]] }).antialias).toBe(3);
  });

  it('reads the old off switch as None and the grades as themselves', () => {
    expect(normalizeInkStroke({ antialias: false, pts: [[1, 2, 1]] }).antialias).toBe(0);
    for (const level of [0, 1, 2, 3]) {
      expect(normalizeInkStroke({ antialias: level, pts: [[1, 2, 1]] }).antialias).toBe(level);
    }
    expect(normalizeInkStroke({ antialias: 9, pts: [[1, 2, 1]] }).antialias).toBe(3);
    expect(normalizeInkStroke({ antialias: -2, pts: [[1, 2, 1]] }).antialias).toBe(0);
  });

  it('reads junk in the anti-alias slot as Strong', () => {
    expect(normalizeInkStroke({ antialias: 'no', pts: [[1, 2, 1]] }).antialias).toBe(3);
    expect(normalizeInkStroke({ antialias: null, pts: [[1, 2, 1]] }).antialias).toBe(3);
  });

  it('reads the flips, the blend, the grain and the corner threshold, and defaults them off', () => {
    const plain = normalizeInkStroke({ pts: [[1, 2, 1]] });
    expect([plain.flipX, plain.flipY, plain.blend, plain.corners]).toEqual(['off', 'off', 'over', 0]);
    expect(plain.texture).toEqual({ on: false, density: 0.5, scale: 100, stress: false });
    const set = normalizeInkStroke({
      flipX: 'on', flipY: 'random', blend: 'density', corners: 400,
      texture: { on: true, density: 7, scale: 1, stress: true }, pts: [[1, 2, 1]],
    });
    expect([set.flipX, set.flipY, set.blend, set.corners]).toEqual(['on', 'random', 'density', 180]);
    expect(set.texture).toEqual({ on: true, density: 1, scale: 10, stress: true });
    const junk = normalizeInkStroke({ flipX: 'yes', blend: 'multiply', texture: 'grain', corners: 'sharp', pts: [[1, 2, 1]] });
    expect([junk.flipX, junk.blend, junk.corners, junk.texture.on]).toEqual(['off', 'over', 0, false]);
  });
...
function canvasOf(w, h) {
  ...
  const over = (i, sr, sg, sb, sa) => {
    ...
    const blend = op === 'darken' ? Math.min : op === 'lighten' ? Math.max : (s) => s;
...
// ===========================================================================
// Phase 7.2: flips, the anti-aliasing grades, the grain, Compare density and
// the pointed corners.
// ===========================================================================

import { sharpenAlpha, texturePixels, densityOver } from './text-paint.js';

describe('flipping the tip', () => {
  // Ink in the tip's left half only, so a flip across the stroke moves it.
  it('mirrors the tip across the stroke when asked, and per stamp at random', () => {
    const straight = footprint(renderTip(dab({ brush: 'half' }), tipMap(HALF)));
    expect(straight.x1).toBeLessThanOrEqual(TX);
    const flipped = footprint(renderTip(dab({ brush: 'half', flipX: 'on' }), tipMap(HALF)));
    expect(flipped.x0).toBeGreaterThanOrEqual(TX - 1);
    // A run of random flips lands some stamps each way. Stamped 20 px apart
    // at 20 px wide the stamps do not touch, so 5 px either side of each
    // centre says which way that stamp went.
    const many = (extra) => renderTip(dab({ brush: 'half', size: 20, spacing: 100, pts: [[20, TY, 1], [TW - 20, TY, 1]], ...extra }), tipMap(HALF));
    const inked = (cnv, x) => at(cnv, x, TY)[3] > 0;
    const rand = many({ flipX: 'random' });
    let left = 0;
    let right = 0;
    for (let c = 20; c <= TW - 20; c += 20) {
      if (inked(rand, c - 5) && !inked(rand, c + 5)) left++;
      if (!inked(rand, c - 5) && inked(rand, c + 5)) right++;
    }
    expect(left).toBeGreaterThan(0);
    expect(right).toBeGreaterThan(0);
    // And the same stroke draws the same random flips twice.
    expect(Array.from(many({ flipX: 'random' }).data)).toEqual(Array.from(rand.data));
  });

  it('mirrors along the stroke on the tip\'s other axis', () => {
    // Ink in the top half only.
    const TOP = tipOf('top', tipImage(32, 32, (x, y) => (y < 16 ? 255 : 0)));
    const up = footprint(renderTip(dab({ brush: 'top' }), tipMap(TOP)));
    expect(up.y1).toBeLessThanOrEqual(TY);
    const down = footprint(renderTip(dab({ brush: 'top', flipY: 'on' }), tipMap(TOP)));
    expect(down.y0).toBeGreaterThanOrEqual(TY - 1);
  });
});

describe('the anti-aliasing grades', () => {
  const ramp = () => {
    const d = new Uint8ClampedArray(256 * 4);
    for (let i = 0; i < 256; i++) d[i * 4 + 3] = i;
    return d;
  };
  const alphas = (d) => Array.from({ length: 256 }, (_, i) => d[i * 4 + 3]);

  it('leaves Strong alone and snaps None at the halfway point', () => {
    expect(alphas(sharpenAlpha(ramp(), 3))).toEqual(alphas(ramp()));
    const none = alphas(sharpenAlpha(ramp(), 0));
    expect(none[127]).toBe(0);
    expect(none[128]).toBe(255);
    expect(none.every((a) => a === 0 || a === 255)).toBe(true);
  });

  it('keeps a narrower band of the ramp at Weak than at Middle', () => {
    const partial = (level) => alphas(sharpenAlpha(ramp(), level)).filter((a) => a > 0 && a < 255).length;
    expect(partial(1)).toBeGreaterThan(0);
    expect(partial(2)).toBeGreaterThan(partial(1));
    expect(partial(3)).toBeGreaterThan(partial(2));
    // Both keep the ends where they are.
    for (const level of [1, 2]) {
      const a = alphas(sharpenAlpha(ramp(), level));
      expect(a[0]).toBe(0);
      expect(a[255]).toBe(255);
      expect(a[128]).toBeGreaterThan(100);
      expect(a[128]).toBeLessThan(160);
    }
  });

  it('cuts a stamped ramp tip harder at Weak than at Strong', () => {
    const partial = (level) => {
      const cnv = renderTip(dab({ brush: 'ramp', antialias: level }), tipMap(RAMP));
      let n = 0;
      for (let i = 3; i < cnv.data.length; i += 4) if (cnv.data[i] > 0 && cnv.data[i] < 255) n++;
      return n;
    };
    expect(partial(0)).toBe(0);
    expect(partial(1)).toBeGreaterThan(0);
    expect(partial(1)).toBeLessThan(partial(3));
    expect(partial(2)).toBeLessThan(partial(3));
    // The old boolean still reads as the two ends.
    expect(partial(false)).toBe(0);
    expect(partial(true)).toBe(partial(3));
  });
});
...
describe('the grain', () => {
  ...
  it('takes ink away in proportion to the density and never adds any', () => {
    const half = texturePixels(solid(64, 64), 64, 64, IDENT, 0, 0, { on: true, density: 0.5, scale: 100 });
    const m = mean(half);
    expect(m).toBeGreaterThan(160);
    expect(m).toBeLessThan(220);
    expect(Math.max(...alphas(half))).toBeLessThanOrEqual(255);
    const full = texturePixels(solid(64, 64), 64, 64, IDENT, 0, 0, { on: true, density: 1, scale: 100 });
    expect(mean(full)).toBeLessThan(m);
    expect(mean(texturePixels(solid(64, 64), 64, 64, IDENT, 0, 0, { on: true, density: 0, scale: 100 }))).toBe(255);
    // Emphasised, the grain bites deeper.
    expect(mean(texturePixels(solid(64, 64), 64, 64, IDENT, 0, 0, { on: true, density: 1, scale: 100, stress: true }))).toBeLessThan(mean(full));
  });

  function alphas(d) {
    const out = [];
    for (let i = 3; i < d.length; i += 4) out.push(d[i]);
    return out;
  }

  it('is fixed to the page, so the same spot grains the same at any zoom', () => {
    // One page px at 1x is a 2 x 2 block at 2x, and the grain must agree.
    // Cells of 4 page px, so no cell boundary falls between a 1x pixel's
    // centre and the centres of the 2x pixels inside it.
    const four = { on: true, density: 1, scale: 800 / 3 };
    const one = texturePixels(solid(32, 32), 32, 32, IDENT, 0, 0, four);
    const two = texturePixels(solid(64, 64), 64, 64, { ...IDENT, a: 2, d: 2 }, 0, 0, four);
    let agree = 0;
    for (let y = 0; y < 32; y++) {
      for (let x = 0; x < 32; x++) {
        if (one[(y * 32 + x) * 4 + 3] === two[(y * 2 * 64 + x * 2) * 4 + 3]) agree++;
      }
    }
    expect(agree).toBe(1024);
    // And the rect's offset is honoured: the same page px read through a
    // rect that starts elsewhere is the same grain.
    const shifted = texturePixels(solid(16, 16), 16, 16, IDENT, 8, 8, four);
    expect(shifted[3]).toBe(one[(8 * 32 + 8) * 4 + 3]);
  });

  it('is coarser at a bigger scale', () => {
    // Count runs of equal alpha along one row: bigger cells, fewer runs.
    const runs = (scale) => {
      const d = texturePixels(solid(128, 1), 128, 1, IDENT, 0, 0, { on: true, density: 1, scale });
      let n = 1;
      for (let x = 1; x < 128; x++) if (d[x * 4 + 3] !== d[(x - 1) * 4 + 3]) n++;
      return n;
    };
    expect(runs(100)).toBeGreaterThan(runs(400));
  });

  it('grains a stroke on the way through the painter', () => {
    const plain = renderTip(dab({ size: 40 }), undefined);
    const grained = renderTip(dab({ size: 40, texture: { on: true, density: 1, scale: 200 } }), undefined);
    expect(mean(grained.data)).toBeLessThan(mean(plain.data));
    // Only alpha moved: the ink is still the stroke's colour.
    expect(at(grained, TX, TY).slice(0, 3)).toEqual([0, 0, 0]);
    // Nothing appears where the plain stroke had nothing.
    for (let i = 3; i < plain.data.length; i += 4) {
      if (plain.data[i] === 0) expect(grained.data[i]).toBe(0);
    }
  });
});

describe('Compare density', () => {
  it('lets the denser of two pixels win and never adds them up', () => {
    const dst = new Uint8ClampedArray([255, 0, 0, 100, 0, 0, 255, 200, 0, 0, 0, 0]);
    const src = new Uint8ClampedArray([0, 255, 0, 150, 0, 255, 0, 150, 0, 255, 0, 150]);
    densityOver(dst, src, 1);
    expect(Array.from(dst)).toEqual([0, 255, 0, 150, 0, 0, 255, 200, 0, 255, 0, 150]);
    // The stroke's opacity goes in on its alpha first.
    const faint = new Uint8ClampedArray([255, 0, 0, 100]);
    densityOver(faint, new Uint8ClampedArray([0, 255, 0, 150]), 0.5);
    expect(Array.from(faint)).toEqual([255, 0, 0, 100]);
  });

  it('keeps two passes of a marker at the marker\'s own opacity', () => {
    const cross = (blend) => {
      const cnv = canvasOf(TW, TH);
      drawInk(
        cnv.getContext('2d'),
        {
          on: true,
          strokes: [
            normalizeInkStroke(dab({ size: 40, opacity: 0.5, blend })),
            normalizeInkStroke(dab({ size: 40, opacity: 0.5, blend, seed: 2 })),
          ],
        },
        (w, h) => canvasOf(w, h),
      );
      return at(cnv, TX, TY)[3];
    };
    expect(cross('over')).toBeGreaterThan(180);
    expect(cross('density')).toBe(128);
  });

  it('is compared against the block\'s own ink, not what is under the block', () => {
    // Paint on a canvas that already holds opaque paper: the stroke must
    // still land, which it would not if it were compared with the paper.
    const cnv = canvasOf(TW, TH);
    const ctx = cnv.getContext('2d');
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(0, 0, TW, TH);
    drawInk(ctx, { on: true, strokes: [normalizeInkStroke(dab({ size: 40, blend: 'density' }))] }, (w, h) => canvasOf(w, h));
    expect(at(cnv, TX, TY)).toEqual([0, 0, 0, 255]);
    expect(at(cnv, 2, 2)).toEqual([255, 255, 255, 255]);
  });
});

describe('a pointed corner', () => {
  // A right angle: right along the top, then down. With the corner pointed
  // the outer corner of the ink is square; without it is rounded to the tip.
  const bend = (extra) => normalizeInkStroke(dab({
    size: 20, spacing: 10, pts: [[40, 30, 1], [100, 30, 1], [100, 90, 1]],
    taperIn: { on: false }, taperOut: { on: false }, ...extra,
  }));
  const paint = (k) => {
    const cnv = canvasOf(TW, TH);
    drawInk(cnv.getContext('2d'), { on: true, strokes: [k] }, (w, h) => canvasOf(w, h));
    return cnv;
  };

  it('fills the outer corner square where the round ends left it rounded', () => {
    const round = paint(bend({}));
    const point = paint(bend({ corners: 45 }));
    // The outer corner of a 20 px line turning at (100, 30) is (110, 20).
    // A round join leaves it clear by 10 - 10/sqrt2 px on each side; the
    // mitre inks it.
    expect(at(round, 108, 22)[3]).toBe(0);
    expect(at(point, 108, 22)[3]).toBe(255);
    // The mitre adds ink only outside the round join, never elsewhere.
    for (let i = 3; i < round.data.length; i += 4) {
      if (round.data[i] === 255) expect(point.data[i]).toBe(255);
    }
    // The stroke's stamps carry it: the corner is a stamp like any other.
    expect(strokeStamps(bend({ corners: 45 })).length).toBeGreaterThan(strokeStamps(bend({})).length);
  });

  it('draws nothing extra on a straight stroke or below the threshold', () => {
    const straight = dab({ size: 20, spacing: 10, pts: [[40, 30, 1], [100, 30, 1], [160, 30, 1]], corners: 45 });
    expect(strokeStamps(normalizeInkStroke(straight)).length).toBe(strokeStamps(normalizeInkStroke({ ...straight, corners: 0 })).length);
    expect(strokeStamps(bend({ corners: 120 })).length).toBe(strokeStamps(bend({})).length);
  });
});

describe('a stroke faded by the opacity dynamics', () => {
  // A hard 20 px line whose points carry an opacity factor: full at the
  // left, a quarter at the right.
  const faded = (extra) => normalizeInkStroke(dab({
    size: 20, spacing: 10, hardness: 100, opacity: 1,
    taperIn: { on: false }, taperOut: { on: false },
    pts: [[40, 30, 1, 1, 1], [160, 30, 1, 0.25, 1]], ...extra,
  }));
  const paint = (k) => {
    const cnv = canvasOf(TW, TH);
    drawInk(cnv.getContext('2d'), { on: true, strokes: [k] }, (w, h) => canvasOf(w, h));
  ...
  it('thins along the stroke and never builds back up where the dabs overlap', () => {
    const cnv = paint(faded({}));
    // Dabs every 2 px overlap tenfold; laid dab over dab the quarter end
    // would read near solid. Densest-wins keeps it a quarter.
    expect(at(cnv, 45, 30)[3]).toBe(255);
    // Densest wins reads a radius behind: the darkest dab reaching x = 155
    // is the one centred 10 px back, at a third strength.
    expect(at(cnv, 155, 30)[3]).toBeLessThan(100);
    expect(at(cnv, 155, 30)[3]).toBeGreaterThan(60);
    const mid = at(cnv, 100, 30)[3];
    expect(mid).toBeGreaterThan(140);
    expect(mid).toBeLessThan(180);
    // The ink keeps the stroke's colour under the fade.
    expect(at(cnv, 155, 30).slice(0, 3)).toEqual(at(cnv, 45, 30).slice(0, 3));
  });

  it('is still a hard edge across the stroke', () => {
    const cnv = paint(faded({}));
    expect(at(cnv, 100, 22)[3]).toBeGreaterThan(100);
    expect(at(cnv, 100, 42)[3]).toBe(0);
  });

  it('draws as before when every factor is 1', () => {
    const plain = paint(faded({ pts: [[40, 30, 1, 1, 1], [160, 30, 1, 1, 1]] }));
    const three = paint(faded({ pts: [[40, 30, 1], [160, 30, 1]] }));
    expect(Array.from(plain.data)).toEqual(Array.from(three.data));
    expect(at(plain, 155, 30)[3]).toBe(255);
  });

  it('a thickness factor squashes the tip across the stroke', () => {
    const squashed = paint(faded({ pts: [[40, 30, 1, 1, 0.4], [160, 30, 1, 1, 0.4]] }));
    const round = paint(faded({ pts: [[40, 30, 1, 1, 1], [160, 30, 1, 1, 1]] }));
    expect(at(round, 100, 22)[3]).toBe(255);
    expect(at(squashed, 100, 22)[3]).toBe(0);
    expect(at(squashed, 100, 28)[3]).toBe(255);
  });
});
