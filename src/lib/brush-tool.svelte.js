...
// Four modes of ONE tool, the same gesture on the same board. Draw lays ink
// down; the other three edit the strokes already there as VECTORS - the
// guide's vector-layer tricks. Erase takes ink out (a whole stroke, the
// touched part, or up to the next crossing - see `eraseMode`); width is CSP's
// Adjust line width, thickening or narrowing what the circle passes over;
// liquify pushes the points themselves, so a bent stroke stays a stroke. The
// box-level liquify (liquify-tool.svelte.js) is a different thing: that one
// bends the placed box's mesh, type and ink together.
export const BRUSH_MODES = ['draw', 'erase', 'width', 'liquify'];

// What the eraser takes: CSP's vector eraser's three options.
export const ERASE_MODES = ['whole', 'part', 'cross'];

// Which way the width tool works.
export const WIDTH_DIRS = ['thicken', 'narrow'];
...
export const brushTool = $state({
  ...
  pen: null,
  // The raw samples of the last stroke drawn on the board, `[{x, y, pressure,
  // t}]`. The panel's preview redraws THIS gesture through the current
  // settings, so a change to a correction or a dynamic is seen on the hand
  // that will use it rather than on a canned wave. Null until one is drawn.
  lastGesture: null,
  // The eraser's reach, one of ERASE_MODES.
  eraseMode: 'whole',
  // The width tool: which way, and how much per pass, percent.
  width: { dir: 'thicken', amount: 20 },
});

export function setEraseMode(mode) {
  if (!ERASE_MODES.includes(mode)) return false;
  brushTool.eraseMode = mode;
  return true;
}
