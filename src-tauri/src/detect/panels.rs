...
mod tests {
    ...
    /// A panel box with its class dropped, which is the shape the golden
    /// fixture records. `detect_classes` is what the engine calls; this pair
    /// exists only to compare against the Python sidecar's `detect_panels`.
    ...
    /// `detect_panels` from the sidecar: detect, keep one class, drop the rest.
    fn detect(session: &mut Session, img: &DynamicImage, class_frame: usize) -> ort::Result<Vec<Panel>> {
        Ok(detect_classes(session, img)?
            .into_iter()
            .filter(|b| b.class == class_frame)
            .map(|b| Panel { x1: b.x1, y1: b.y1, x2: b.x2, y2: b.y2 })
            .collect())
    }
