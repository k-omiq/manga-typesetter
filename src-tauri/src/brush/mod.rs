...
mod tests {
    ...
    /// The corpus, or `None` when this checkout does not carry it.
    ///
    /// `external/` is not in the repository - the `.sut` files are third-party
    /// brush packs - so on CI and on a fresh clone there is nothing here to
    /// test against and these skip, exactly as the detection goldens do. When
    /// the corpus IS present it must be whole: a partial one would quietly
    /// narrow the regression suite instead of failing.
    fn corpus_or_skip() -> Option<Vec<PathBuf>> {
        let files = corpus();
        if files.is_empty() {
            eprintln!("skipping: no .sut corpus under external/ in this checkout");
            return None;
        }
    ...
    fn the_json_the_library_receives_is_camel_case_and_complete() {
        ...
        assert_eq!(b["settings"]["antialias"], 3, "a grade, not a flag");
        assert_eq!(b["settings"]["blend"], "over");
        assert_eq!(b["settings"]["flipX"], "off");
        ...
        assert!(!set.contains(&"texture"), "no texture in the file, no key");
        ...
        // Its two siblings are conditional in exactly the same way, and most of
        // the corpus never carries them at all, so their absence here is the
        // ordinary case rather than a gap in the round tip.
        assert!(!set.contains(&"dynOpacity"), "no decoded opacity dynamics, no key");
        assert!(!set.contains(&"dynThick"), "no decoded thickness dynamics, no key");
        ...
        // And when a file does carry them they travel under the camel-case
        // names `src/lib/brush.js` reads, not under `dyn_thick`.
        let thick = round_brush(
            "abc".into(),
            "round".into(),
            &BrushSettings {
                dyn_thick: Some(variant::SizeDynamics {
                    src: effector::Source::Random,
                    amount: 76.0,
                    curve: None,
                }),
                ..BrushSettings::default()
        ...
        .unwrap();
        let t = &serde_json::to_value(thick).unwrap()["settings"]["dynThick"];
        assert_eq!(t["src"], "random");
        assert_eq!(t["amount"], 76.0);
    ...
    fn one_bad_path_does_not_cost_the_good_ones() {
        let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let Some(files) = corpus_or_skip() else { return };
        let good = files.into_iter().next().expect("the corpus is present");
        let paths = vec![
    ...
    fn every_corpus_brush_imports_with_settings_and_a_readable_tip() {
        ...
        let mut pixels = 0;
        let mut round = 0;
        let (mut textured, mut density) = (0, 0);
        ...
        for path in &files {
            ...
            let first = &imported[0].settings;
            if first.texture.is_some() {
                textured += 1;
            }
            if first.blend == variant::BlendMode::Density {
                density += 1;
            }
            for b in imported {
                brushes += 1;
        ...
        // Measured off the corpus by hand before the columns were read: five
        // files name CSP's stock noise texture, twenty carry Compare density.
        assert_eq!(textured, 5, "five textured files");
        assert_eq!(density, 20, "twenty Compare density files");
    ...
    /// The other two `Effector` blobs, the same way round. These are the columns
    /// a later CSP added, so most of the corpus does not have them at all, and
    /// "not there" and "there but switched off" must both read as no key: a
    /// brush that quietly gained opacity dynamics it was never authored with
    /// would ink every stroke differently and nothing would say why.
    #[test]
    fn every_corpus_brush_arrives_with_the_opacity_and_thickness_its_file_carries() {
        let mut opacity = std::collections::BTreeMap::new();
        let mut thick = std::collections::BTreeMap::new();
        let Some(files) = corpus_or_skip() else { return };
        for path in files {
            let name = path.file_name().unwrap_or_default().to_string_lossy().into_owned();
            let imported = import_file(&path).expect("the corpus imports clean");
            // One `Variant` row per file, so as with size these must be the same
            // across every tip the file yields rather than read per brush.
            let first = imported[0].settings.clone();
            for b in &imported {
                assert_eq!(b.settings.dyn_opacity, first.dyn_opacity, "{name}: two opacities");
                assert_eq!(b.settings.dyn_thick, first.dyn_thick, "{name}: two thicknesses");
            }
            for d in [&first.dyn_opacity, &first.dyn_thick].into_iter().flatten() {
                assert!((0.0..=100.0).contains(&d.amount), "{name}: amount {}", d.amount);
                assert_ne!(d.src, effector::Source::Tilt, "{name}: the engine has no tilt");
            }
            *opacity.entry(format!("{:?}", first.dyn_opacity.map(|d| d.src))).or_insert(0) += 1;
            *thick.entry(format!("{:?}", first.dyn_thick.map(|d| d.src))).or_insert(0) += 1;
        }
        // Measured off the corpus's blobs with sqlite before the columns were
        // read, so a disagreement here is this module's reading and not the
        // files': not one of the 64 drives its opacity off anything.
        assert_eq!(opacity.get("None"), Some(&64), "opacity tally was {opacity:?}");
        // Thickness is nearly as rare: two brushes squash their tip at random,
        // and one drives it off velocity - that file switches velocity and
        // random on together, and `primary()` hands over velocity.
        assert_eq!(thick.get("Some(Random)"), Some(&2), "thickness tally was {thick:?}");
        assert_eq!(thick.get("Some(Velocity)"), Some(&1), "thickness tally was {thick:?}");
        assert_eq!(thick.get("None"), Some(&61), "thickness tally was {thick:?}");
        assert_eq!(thick.values().sum::<i32>(), 64, "one reading per corpus file");
        println!("corpus opacity: {opacity:?}, thickness: {thick:?}");
    }
}
